...
async function run({ port, mainAPI }) {
  ...
  try {
    ...
    const deadline = Date.now() + Math.min(120000, Number(process.env.COURTFLOW_PROBE_WAIT_MS) || 75000);
    while (Date.now() < deadline && imageCount < 5) {
      ...
      if (state.state === 'failed' || state.state === 'confirmed' || state.submissionAttempted) throw new Error(`Unexpected read-only state: ${state.state}: ${state.statusText}`);
      // The first readable frame may precede preparation of the reservation form.
      // Wait for that separate step rather than treating it as a missing court.
      if (!state.frameReady || !state.selectedCourtId) { startupPolls++; await pause(350); continue; }
      if (state.selectedCourtId !== slot.courtId) throw new Error('A different court was selected');
    ...
    if (imageCount !== 5) throw new Error('Timed out before five decoded exact-court frames');
